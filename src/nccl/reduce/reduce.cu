...
template<typename T>
void run_reduce(size_t local_count, size_t global_count, int size, int rank, NcclContext& ctx, const std::string& data_type) {
    // set root for reduce
    int root = 0;
    ...
    MPI_Bcast(&ok, 1, MPI_C_BOOL, root, MPI_COMM_WORLD);
...
int main(int argc, char* argv[]) {
    ...
    if (dtype == "int") {
        run_reduce<int>(local_count, effective_global, size, rank, ctx, dtype);
    } else if (dtype == "float") {
        run_reduce<float>(local_count, effective_global, size, rank, ctx, dtype);
    } else if (dtype == "double") {
        run_reduce<double>(local_count, effective_global, size, rank, ctx, dtype);
    } else {
