// init_time.cu — Dedicated benchmark for NCCL communicator initialization time.
//
// Design:
...
//   - Warmup block (untimed): one init+destroy cycle before the measurement to
//     pre-load CUDA/NCCL shared libraries within the process.
//
// Usage (called from run_init_time.sh):
//   mpirun -np 4 ./build/nccl/init_time --iter 1 --output results/...
...
int main(int argc, char* argv[]) {
    ...
    if (rank == 0 && !output_dir.empty()) {
        ...
        csv_path = output_dir + "/nccl_init_time_"
