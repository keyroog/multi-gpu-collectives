// init_time.cpp — Dedicated benchmark for RCCL communicator initialization time.
//
// Design: identical to the NCCL version, with CUDA → HIP API substitutions.
//
...
//   - MPI_Barrier before timer start: all ranks start the clock simultaneously.
//   - ncclGetUniqueId + MPI_Bcast + ncclCommInitRank: the full "connect" phase.
//   - MPI_Barrier after init: timer stops when the LAST rank has finished.
//   - MPI_Gather of per-rank times to rank 0: rank 0 computes and logs the true max.
//   - Warmup block (untimed): one init+destroy cycle before the measurement.
//
// Usage (called from run_init_time.sh):
//   mpirun -np 4 ./build/rccl/init_time --iter 1 --output results/...
...
int main(int argc, char* argv[]) {
    ...
    if (ndev == 0) {
        std::cerr << "[Rank " << rank << "] No HIP devices found\n";
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    hipSetDevice(local_rank % ndev);
    ...
    if (rank == 0 && !output_dir.empty()) {
        ...
        csv_path = output_dir + "/rccl_init_time_"
    ...
    double local_ms = (t_end - t_start) * 1000.0;
