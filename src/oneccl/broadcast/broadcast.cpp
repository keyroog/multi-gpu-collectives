...
void run_broadcast(size_t local_count, size_t global_count, int size, int rank, ccl::communicator& comm, sycl::queue& q, ccl::stream stream, 
    ...
    ccl::broadcast((void*)buf, local_count, ccl_dtype, root_rank, comm, stream).wait();
...
int main(int argc, char* argv[]) {
    ...
    if (dtype == "int") {
        run_broadcast<int>(local_count, effective_global_count, size, rank, comm, q, stream, logger, dtype, root_rank, ccl::datatype::int32);
    } else if (dtype == "float") {
        run_broadcast<float>(local_count, effective_global_count, size, rank, comm, q, stream, logger, dtype, root_rank, ccl::datatype::float32);
    } else if (dtype == "double") {
        run_broadcast<double>(local_count, effective_global_count, size, rank, comm, q, stream, logger, dtype, root_rank, ccl::datatype::float64);
    } else {
