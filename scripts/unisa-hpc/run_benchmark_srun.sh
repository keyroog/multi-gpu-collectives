#!/bin/bash
# run_benchmark_srun.sh - Esegue i benchmark per una collettiva, libreria e dtype specifici
#
# Usage: ./run_benchmark_srun.sh <library> <collective> <dtype>
# Esempio: ./run_benchmark_srun.sh nccl allreduce float
# Multi-nodo: PPN=4 NUM_GPUS=8 ./run_benchmark_srun.sh rccl allreduce float
#
# NOTA: Eseguire all'interno di una sessione interattiva con allocazione SLURM, dopo aver fatto:
#   source /home/S.SIRICA3/scripts/export_variables.sh
#
# Variabili di ambiente opzionali:
#   NUM_GPUS     - Numero di GPU/rank (default: 4)
#   NUM_ITERS    - Numero di iterazioni per ogni size (default: 5)
#   PPN          - Processi per nodo per multi-nodo (se impostato, aggiunge --ntasks-per-node a srun)
...
# ===================== Costruzione comando srun =====================
SRUN_ARGS="--ntasks $NUM_GPUS"
if [ -n "$PPN" ]; then
    SRUN_ARGS="--ntasks-per-node $PPN $SRUN_ARGS"
fi
...
echo " srun: srun ${SRUN_ARGS} ${BINARY} ..."
...
for i in "${!SIZES_BYTES[@]}"; do
    ...
    for iter in $(seq 1 "$NUM_ITERS"); do
        ...
        if ! srun $SRUN_ARGS "$BINARY" --dtype "$DTYPE" --count "$COUNT" --output "$OUTPUT_DIR" 2>&1; then
