#!/usr/bin/env python3
"""
Genera grafici di confronto NCCL vs MPI GPU-aware per ogni coppia (collettiva, dtype).

Usage:
    python scripts/python/plot_comparison_nccl_mpi.py
    python scripts/python/plot_comparison_nccl_mpi.py --results-dir results/leonardo/4_rank --out-dir plots/leonardo/nccl_vs_mpi
"""
import os
import glob
import argparse
...
LIBRARY_STYLE = {
    ...
    "mpi":  {"color": "#9467bd", "marker": "^", "label": "MPI GPU-aware"},
}
...
def load_all_results(results_dir: str) -> pd.DataFrame:
    """Carica tutti i CSV da results_dir/{nccl,mpi}/{collective}/."""
    frames = []
    for library in ("nccl", "mpi"):
...
def main():
    parser = argparse.ArgumentParser(
        description="Genera grafici di confronto NCCL vs MPI GPU-aware per collettiva e dtype."
    )
    parser.add_argument(
        "--results-dir",
    ...
    parser.add_argument(
        "--out-dir",
        default="plots/leonardo/nccl_vs_mpi",
        help="Directory di output per i grafici (default: plots/leonardo/nccl_vs_mpi)",
    )
    parser.add_argument(
        "--format",
        dest="formats",
